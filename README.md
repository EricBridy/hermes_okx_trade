...
| 评分要素 | 分值 | 说明 |
|----------|------|------|
| 15m趋势一致性 | 一票否决 | 5m方向和15m趋势相反 → 直接0分不开仓 |
| RSI极端区域 | 一票否决 | RSI>70不做多，RSI<30不做空 |
| 1m反转检测 | 一票否决 | 最近3根1m和5m方向相反 → 不开仓 |
...
| 最大持仓 | 5个 | 通道A和通道B各最多5个 |

### 仓位动态分配

- 无空位时，评分最高的候选可**淘汰评分最低的持仓**（高分替换低分）
- 有空位时，按权重动态分配余额（总仓位≤95%）
