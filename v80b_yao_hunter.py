#!/usr/bin/env python3
"""
OKX 妖币猎手 v8.0B — 剥头皮反转（路线B）
目标：胜率 75-85%，盈亏比 1:2（小止盈大止损），高频快进快出
核心理念：只在极端FR反向做剥头皮，TP小SL大，快速拿回

v8.0B 核心变化：
- 仅通道A（极端FR反向），关闭通道B
- TP 0.8% / SL 2.5%（小止盈大止损，胜率优先）
- |FR| > 0.15% 才触发（只做最极端的FR）
- 持仓 3-8 分钟就平（快进快出）
- 追踪止损: 浮盈0.5%激活, 距离0.3%（快速锁利）
- 每日交易上限: 15 笔
- 扫描池 top60
- 24小时运行
"""
...
# ==================== 配置 ====================
LEVERAGE = 6
TP_PCT = 0.008         # 0.8% 止盈（小止盈快速拿回）
SL_PCT = 0.025         # 2.5% 止损（大止损避免被震出）
TRAIL_ACTIVATE = 0.005  # 浮盈0.5%激活追踪
TRAIL_DISTANCE = 0.003  # 追踪止损距离0.3%（快速锁利）
TIME_STOP_SEC = 180       # 3分钟: 浮亏>0.5%止损
TIME_STOP_P3_SEC = 480    # 8分钟: 横盘止损
SCAN_INTERVAL = 20      # 20秒扫描（更快）
COOLDOWN_SEC = 600      # 同一品种冷却10分钟
MAX_CONCURRENT = 2
MAX_CONSECUTIVE_LOSSES = 4
LOSS_PAUSE_SEC = 1200   # 连亏暂停20分钟
SWAP_COOLDOWN = 600     # 换仓冷却10分钟

# v8.0B: 每日交易上限
MAX_DAILY_TRADES = 15

# 通道A: 极端FR（仅通道A，关闭通道B）
FR_EXTREME_THRESHOLD = 0.0015   # |FR| > 0.15% 才触发（只做最极端）
FR_MAX_THRESHOLD = 0.01
CHANNEL_A_VOL_SPIKE = 1.2       # 放量阈值（稍宽松）
CHANNEL_A_POSITION_PCT = 0.50   # 仓位占总资金50%（单通道更大仓位）

# 通道B: 关闭
MOMENTUM_SCORE_THRESHOLD = 99   # 设为不可能达到的值=关闭通道B
CHANNEL_B_POSITION_PCT = 0.0
...
STATE_FILE = os.path.join(SCRIPT_DIR, "v80b_state.json")
LOG_FILE = os.path.join(SCRIPT_DIR, "v80b_trades.log")
CHAIN_CACHE_FILE = os.path.join(SCRIPT_DIR, "v80b_chain_cache.json")
...
def monitor_position(inst_id, pos_info):
    ...
    if elapsed > TIME_STOP_SEC:
        # 3分钟后: 浮亏>0.5% → 快速止损
        if pct_change < -0.5:
            log(f"⏰ {inst_id} P1: 3分钟浮亏{pct_change:+.2f}% → 快速止损")
            close_position(inst_id, pos_info.get("algo_ids"))
            return "TIME_STOP"
    ...
    if elapsed > TIME_STOP_P3_SEC:
        # 8分钟后: 任何浮盈<0.3% → 横盘出局
        if pct_change < 0.3:
            log(f"⏰ {inst_id} P3: 8分钟横盘{pct_change:+.2f}% → 出局")
            close_position(inst_id, pos_info.get("algo_ids"))
            return "TIME_STOP"
        # 浮盈>0.3%且未激活追踪 → 激活
        if not pos_info.get("trail_activated"):
            pos_info["trail_activated"] = True
            pos_info["highest_pnl_pct"] = pct_change
            log(f"🔔 {inst_id} 8分钟+浮盈{pct_change:+.2f}% → 追踪激活")
...
def main():
    log("=" * 60)
    log("🔥 OKX 妖币猎手 v8.0B 启动（剥头皮反转 — 高胜率快进快出）")
    log(f"  杠杆: {LEVERAGE}x  TP: {TP_PCT*100}%  SL: {SL_PCT*100}%")
    log(f"  仅通道A: |FR|>{FR_EXTREME_THRESHOLD*100}% 极端FR反向 仓位{CHANNEL_A_POSITION_PCT*100:.0f}%")
    log(f"  v8.0B: 小止盈大止损 | 快进快出3-8分钟 | 每日上限{MAX_DAILY_TRADES}笔")
    log(f"  追踪止损: 激活{TRAIL_ACTIVATE*100}% 距离{TRAIL_DISTANCE*100}%")
    log(f"  时间止损: P1={TIME_STOP_SEC}s(浮亏>0.5%) P3={TIME_STOP_P3_SEC}s(横盘<0.3%)")
