#!/usr/bin/env python3
"""
OKX 妖币猎手 v8.0A — 极端选择性（路线A）
目标：胜率 65-75%，盈亏比 2:1+，日交易量 3-8 笔
核心理念：少而精，只开"完美信号"，大部分时间空仓

v8.0A 核心变化（基于v7.9回测复盘）：
- 评分门槛 10→18（仅最强信号）
- 硬过滤全面回归（EMA/Stoch/5m量能/ADX/BB全部一票否决）
- 链上强制要求：至少2个链上因子命中（SM/HOT/INF/社/鲸）
- 删除 TIME_P2（避免把正确方向的仓位震出去）
- TIME_P1 放宽到 6分钟浮亏>1.2% 才止损
- 追踪止损: 浮盈1.5%激活, 距离0.8%（保护大利润）
- 每日交易上限: 8 笔
- 同币冷却: 4 小时
- 黑名单机制: 连续3次亏损 → 24h拉黑
- 扫描池 top80→top40（减少噪声）
- 开仓时段限制: UTC+8的08-12/20-24（亚洲+欧美活跃期）
"""
...
TP_PCT = 0.03          # 3% 止盈
SL_PCT = 0.015         # 1.5% 止损
TRAIL_ACTIVATE = 0.015  # 浮盈1.5%激活追踪（保护大利润）
TRAIL_DISTANCE = 0.008  # 追踪止损距离0.8%
TIME_STOP_SEC = 360       # 6分钟 phase1 时间止损
TIME_STOP_P3_SEC = 900    # 15分钟 phase3 横盘止损
SCAN_INTERVAL = 30
COOLDOWN_SEC = 14400     # 同一品种冷却4小时
MAX_CONCURRENT = 2
MAX_CONSECUTIVE_LOSSES = 3
LOSS_PAUSE_SEC = 1800
SWAP_COOLDOWN = 3600     # 换仓冷却1小时

# v8.0A: 每日交易上限
MAX_DAILY_TRADES = 8

# v8.0A: 黑名单机制
BLACKLIST_LOSS_COUNT = 3   # 连续亏损次数
BLACKLIST_DURATION = 86400 # 拉黑时长24h

# 通道A: 极端FR
FR_EXTREME_THRESHOLD = 0.0008   # |FR| > 0.08% 触发（更严格）
FR_MAX_THRESHOLD = 0.01
CHANNEL_A_VOL_SPIKE = 1.5       # 放量阈值（更严格）
CHANNEL_A_POSITION_PCT = 0.40

# 通道B: 动量顺势
MOMENTUM_SCORE_THRESHOLD = 18   # v8.0A: 评分>=18（极严）
CHANNEL_B_POSITION_PCT = 0.30

# v8.0A: 链上强制要求
MIN_CHAIN_FACTORS = 2  # 至少2个链上因子命中

# 共用
MIN_24H_VOL = 5000000           # 最小24h成交量 $500万（更严）
MIN_PRICE = 0.001
OKX_TAKER_FEE = 0.0005
ROUND_TRIP_FEE = OKX_TAKER_FEE * 2

# v8.0A: 开仓时段限制（UTC+8）- 仅亚洲+欧美活跃期
TRADING_WINDOWS = [(8, 0, 12, 0), (20, 0, 23, 59)]
...
# v8.0A: 黑名单 + 每日统计
BLACKLIST = {}      # symbol -> unblock_timestamp
LOSS_STREAK = {}    # symbol -> consecutive loss count
DAILY_TRADE_COUNT = {"date": "", "count": 0}  # 每日交易计数
...
STATE_FILE = os.path.join(SCRIPT_DIR, "v80a_state.json")
LOG_FILE = os.path.join(SCRIPT_DIR, "v80a_trades.log")
CHAIN_CACHE_FILE = os.path.join(SCRIPT_DIR, "v80a_chain_cache.json")
...
def monitor_position(inst_id, pos_info):
    ...
    # v8.0A: 分阶段时间止损（删除 TIME_P2，避免把正确方向震出去）
    if elapsed > TIME_STOP_SEC:
        # Phase1: 6分钟 + 浮亏>1.2% → 快速止损
        if pct_change < -1.2:
            log(f"⏰ {inst_id} P1: 6分钟浮亏{pct_change:+.2f}%>1.2% → 快速止损")
            close_position(inst_id, pos_info.get("algo_ids"))
            return "TIME_P1"

    if elapsed > TIME_STOP_P3_SEC:
        # Phase3: 15分钟 + 浮盈<0.3% → 横盘止损
        if pct_change < 0.3:
            log(f"⏰ {inst_id} P3: 15分钟横盘{pct_change:+.2f}% → 出局")
            close_position(inst_id, pos_info.get("algo_ids"))
            return "TIME_P3"
        # 浮盈>0.5%且未激活追踪 → 激活保本追踪
        if pct_change >= 0.5 and not pos_info.get("trail_activated"):
            pos_info["trail_activated"] = True
            pos_info["highest_pnl_pct"] = pct_change
            log(f"🔔 {inst_id} 15分钟+浮盈{pct_change:+.2f}% → 保本追踪激活")
...
def main():
    log("=" * 60)
    log("🔥 OKX 妖币猎手 v8.0A 启动（极端选择性 — 少而精）")
    ...
    log(f"  通道B: 动量>={MOMENTUM_SCORE_THRESHOLD}+链上>={MIN_CHAIN_FACTORS}因子+EMA+5m量能+ADX+BB+ROC+Stoch 仓位{CHANNEL_B_POSITION_PCT*100:.0f}%")
    log(f"  v8.0A: 评分门槛18 | 链上强制{MIN_CHAIN_FACTORS}因子 | 扫描top{SCAN_TOP_N} | 每日上限{MAX_DAILY_TRADES}笔")
    log(f"  追踪止损: 激活{TRAIL_ACTIVATE*100}% 距离{TRAIL_DISTANCE*100}%")
    log(f"  时间止损: P1={TIME_STOP_SEC}s(浮亏>1.2%) P3={TIME_STOP_P3_SEC}s(横盘<0.3%)")
        ...
        try:
            ...
            if positions:
                for inst_id in list(positions.keys()):
                    ...
                    if result in ("PROFIT", "TRAIL_STOP", "TIME_P1", "TIME_P3", "CLOSED"):
                        # 先保存upl（后续del会清除）
                        ...
                        if result in ("PROFIT", "TRAIL_STOP"):
                            ...
                            state["trade_count"] = state.get("trade_count", 0) + 1
                        elif result in ("TIME_P1", "TIME_P3"):
                            if saved_upl > 0:
                                state["consecutive_losses"] = 0
                            else:
                                state["consecutive_losses"] = state.get("consecutive_losses", 0) + 1
                            state["total_pnl"] = state.get("total_pnl", 0) + saved_upl
                            state["trade_count"] = state.get("trade_count", 0) + 1
                            record_trade_result(inst_id, saved_upl)
                            if state["consecutive_losses"] >= MAX_CONSECUTIVE_LOSSES:
                                pause_until = datetime.now() + timedelta(seconds=LOSS_PAUSE_SEC)
                                state["pause_until"] = pause_until.isoformat()
                                log(f"⏸️ {state['consecutive_losses']}连亏，暂停{LOSS_PAUSE_SEC//60}分钟")
                ...
                if channel_a and balance >= 1:
                    ...
                    if top_a:
                        ...
                        if check_daily_limit():
                            log(f"  ⚠️ 今日已达{MAX_DAILY_TRADES}笔上限")
                        elif check_blacklist(top_a["sym"]):
                            log(f"  ⚠️ {top_a['sym']} 在黑名单中")
                        else:
            ...
            if slot2_id:
                ...
                if channel_b and balance >= 1:
                    ...
                    if top_b:
                        ...
                        if check_daily_limit():
                            log(f"  ⚠️ 今日已达{MAX_DAILY_TRADES}笔上限")
                        elif check_blacklist(top_b["sym"]):
                            log(f"  ⚠️ {top_b['sym']} 在黑名单中")
                        else:
