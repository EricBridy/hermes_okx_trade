#!/usr/bin/env python3
"""
OKX 妖币猎手 v8.0E — 降噪增强版（基于v80d + 虚拟币专属优化）
目标：胜率 60-70%，盈亏比 1.5:1，日交易量 8-15 笔
核心理念：多TF加权评分 + Bar Close确认 + ATR动态阈值 + 插针过滤

v8.0E 核心降噪技术（基于MQL5案例42%→61%胜率提升）：
- 多TF RSI加权评分（1m 20% + 5m 30% + 15m 50%）替代硬性阈值
- 15m用Heikin-Ashi平均K线做趋势过滤（其他TF保留原始K线）
- Bar Close确认：15m趋势用已收盘K线数据（避免正在形成的K线骗信号）
- ATR动态阈值：波动率大时RSI放宽到75/25，小时收紧到65/35
- 插针过滤：1m K线的影线/实体比>3:1 视为插针，跳过开仓
- 保留v80d所有优化：TP2.5% SL1.5% 无TIME_P2 P3=20分钟

继承v80d的参数（TP/SL/冷却/黑名单等）
"""
...
STATE_FILE = os.path.join(SCRIPT_DIR, "v80e_state.json")
LOG_FILE = os.path.join(SCRIPT_DIR, "v80e_trades.log")
CHAIN_CACHE_FILE = os.path.join(SCRIPT_DIR, "v80e_chain_cache.json")
...
def calculate_momentum_score(chg_5m, vol_1m, ups, downs, chain_bonus, fr, trend_15m=None, chg_5m_dir=None, mt_data=None):
    """通道B综合评分 v8.0E（多TF加权RSI + ATR动态阈值 + 插针过滤）"""
    score = 0
...
def main():
    log("=" * 60)
    log("🔥 OKX 妖币猎手 v8.0E 启动（降噪增强 — 多TF加权+HA+Bar Close+ATR+插针过滤）")
    ...
    log(f"  v8.0E: 多TF RSI(1m 20%+5m 30%+15m 50%) | 15m HA+Bar Close | ATR动态 | 插针过滤")
