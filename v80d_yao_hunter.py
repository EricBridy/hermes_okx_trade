#!/usr/bin/env python3
"""
OKX 妖币猎手 v8.0D — 高频均值回归（基于v80c优化）
目标：胜率 55-65%，盈亏比 1.2:1，日交易量 8-15 笔
核心理念：FR均值回归是最强信号，小TP快速兑现，删除TIME_P2避免误杀

v8.0D 核心变化（基于v80c回测+实盘数据）：
- TP 3%→1.8%（更容易触发，提高胜率）
- SL 保持 1.5%
- 删除 TIME_P2（v80c回测中TIME_P2是最大亏损来源：29笔全亏-$7.85）
- TIME_P3 从15分钟延长到20分钟（给更多时间到TP）
- FR门槛从 0.06%→0.05%（更多开仓机会）
- 链上因子要求从1→0（FR本身就是最强信号）
- 冷却时间从2小时→1小时
- 评分门槛从13→10
- 追踪止损: 浮盈1%激活, 距离0.5%（更快锁利）
- 每日交易上限: 25笔
"""
...
# 通道B: 动量顺势
MOMENTUM_SCORE_THRESHOLD = 10   # v8.0D: 评分>=10（放宽）
...
STATE_FILE = os.path.join(SCRIPT_DIR, "v80d_state.json")
LOG_FILE = os.path.join(SCRIPT_DIR, "v80d_trades.log")
CHAIN_CACHE_FILE = os.path.join(SCRIPT_DIR, "v80d_chain_cache.json")
...
def monitor_position(inst_id, pos_info):
    ...
    # v8.0D: 分阶段时间止损（删除TIME_P2，避免误杀正确方向仓位）
    ...
    if elapsed > TIME_STOP_P3_SEC:
        ...
        if pct_change >= 0.5 and not pos_info.get("trail_activated"):
            pos_info["trail_activated"] = True
            pos_info["highest_pnl_pct"] = pct_change
            log(f"🔔 {inst_id} 20分钟+浮盈{pct_change:+.2f}% → 保本追踪激活")
...
def main():
    log("=" * 60)
    log("🔥 OKX 妖币猎手 v8.0D 启动（高频均值回归 — FR信号优先+快速止盈）")
    ...
    log(f"  通道B: 动量>={MOMENTUM_SCORE_THRESHOLD}+EMA+5m量能 仓位{CHANNEL_B_POSITION_PCT*100:.0f}%")
    log(f"  v8.0D: TP1.8% | 无TIME_P2 | P3=20分钟 | FR门槛0.05% | 链上不强制 | 每日上限{MAX_DAILY_TRADES}笔")
