#!/usr/bin/env python3
"""
OKX 妖币猎手 v8.0C — 混合路线（链上优先+中等频率）
目标：胜率 55-65%，盈亏比 2:1，日交易量 10-20 笔
核心理念：链上信号优先，技术面确认，中等频率中等胜率

v8.0C 核心变化：
- 评分门槛 10→13（适度提高）
- 链上至少1个因子命中（SM/HOT/INF/社/鲸）
- 保留 TIME_P2 但放宽到 10 分钟
- 追踪止损: 浮盈1.2%激活, 距离0.7%
- 每日交易上限: 20 笔
- 同币冷却: 2 小时
- 扫描池 top60
...
TP_PCT = 0.03          # 3% 止盈
SL_PCT = 0.015         # 1.5% 止损
TRAIL_ACTIVATE = 0.012  # 浮盈1.2%激活追踪
TRAIL_DISTANCE = 0.007  # 追踪止损距离0.7%
TIME_STOP_SEC = 360       # 6分钟 phase1: 浮亏>1%止损
TIME_STOP_P2_SEC = 600    # 10分钟 phase2: 任何浮亏止损（放宽）
TIME_STOP_P3_SEC = 900    # 15分钟 phase3: 横盘止损
SCAN_INTERVAL = 30
COOLDOWN_SEC = 7200      # 同一品种冷却2小时
MAX_CONCURRENT = 2
...
# 通道A: 极端FR
FR_EXTREME_THRESHOLD = 0.0006   # |FR| > 0.06% 触发
FR_MAX_THRESHOLD = 0.01
CHANNEL_A_VOL_SPIKE = 1.3
CHANNEL_A_POSITION_PCT = 0.40

# 通道B: 动量顺势
MOMENTUM_SCORE_THRESHOLD = 13   # v8.0C: 评分>=13（适度提高）
...
STATE_FILE = os.path.join(SCRIPT_DIR, "v80c_state.json")
LOG_FILE = os.path.join(SCRIPT_DIR, "v80c_trades.log")
CHAIN_CACHE_FILE = os.path.join(SCRIPT_DIR, "v80c_chain_cache.json")
...
def monitor_position(inst_id, pos_info):
    ...
    if elapsed > TIME_STOP_P2_SEC and pct_change < 0:
        log(f"⏰ {inst_id} P2: {int(elapsed)}s 浮亏{pct_change:+.2f}% → 时间止损")
        close_position(inst_id, pos_info.get("algo_ids"))
        return "TIME_STOP"

    if elapsed > TIME_STOP_P3_SEC:
        if pct_change < 0.3:
            log(f"⏰ {inst_id} P3: {int(elapsed)}s 横盘{pct_change:+.2f}% → 出局")
            close_position(inst_id, pos_info.get("algo_ids"))
            return "TIME_STOP"
...
def main():
    log("=" * 60)
    log("🔥 OKX 妖币猎手 v8.0C 启动（混合路线 — 链上优先+中等频率）")
    log(f"  杠杆: {LEVERAGE}x  TP: {TP_PCT*100}%  SL: {SL_PCT*100}%")
    log(f"  通道A: |FR|>{FR_EXTREME_THRESHOLD*100}%+技术验证 仓位{CHANNEL_A_POSITION_PCT*100:.0f}%")
    log(f"  通道B: 动量>={MOMENTUM_SCORE_THRESHOLD}+链上>={MIN_CHAIN_FACTORS}因子+EMA+5m量能 仓位{CHANNEL_B_POSITION_PCT*100:.0f}%")
    log(f"  v8.0C: 评分门槛13 | 链上{MIN_CHAIN_FACTORS}因子 | 扫描top{SCAN_TOP_N} | 每日上限{MAX_DAILY_TRADES}笔")
    log(f"  追踪止损: 激活{TRAIL_ACTIVATE*100}% 距离{TRAIL_DISTANCE*100}%")
    log(f"  时间止损: P1={TIME_STOP_SEC}s(亏>1%) P2={TIME_STOP_P2_SEC}s(任何亏) P3={TIME_STOP_P3_SEC}s(横盘)")
